...
API tokens allow you to call WHM API 1 functions outside of a WHM session. You can use API tokens instead of a password to call WHM API 1 functions over `HTTPS`. This is useful, for example, to allow a reseller user or third-party developer to run API function calls with your account's data.

**Important:**

API calls using a method that includes a URL must use one the following ports:

* `2087` - Secure calls to WHM's APIs, or to cPanel's APIs via the WHM API 1.
* `443` - Secure calls to WHM's APIs, or to cPanel's APIs via the WHM API 1 through a [service subdomain](https://go.cpanel.net/ServiceProxySubdomains).


## Creating an API token

To create an API token, use WHM's [*Manage API Tokens*](https://go.cpanel.net/whmdocsManageasisAPITokens) interface (*WHM >> Home >> Development >> Manage API Tokens*). You can also use the WHM API 1 [`api_token_create`](/openapi/whm/operation/api_token_create/) function.

**Note:**

* In cPanel & WHM version 82, when an API token expires, the system will **not** remove it. You must manually delete an API token.
* You can remove an API token with WHM's [*Manage API Tokens*](https://go.cpanel.net/whmdocsManageasisAPITokens) interface (*WHM >> Home >> Development >> Manage API Tokens*) or the WHM API 1 [`api_token_revoke`](/openapi/whm/operation/api_token_revoke/) function.
...
* Line 16 performs the WHM API 1 [`listaccts`](/openapi/whm/operation/listaccts/) function via a `curl` call and saves the response in the `$response` variable.
* The script prints the following output:
  * If the [`listaccts`](/openapi/whm/operation/listaccts/) function succeeds, it parses the JSON data and returns the account's usernames.
  * If the [`listaccts`](/openapi/whm/operation/listaccts/) function fails, it returns an error message.
