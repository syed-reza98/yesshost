      ...
      if (res.ok) {
        ...
        if (targetInvoice) {
          ...
          if (match && targetAction === "pay" && match.status !== "paid") {
            toast.info(bn ? `ইনভয়েস #${match.invoiceNumber} পরিশোধের জন্য প্রস্তুত` : `Invoice #${match.invoiceNumber} ready for payment`);
          }
        }
      }
    } catch {
  ...
  const handleShare = async (inv: InvoiceItem) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(bn ? "পাবলিক লিংক কপি হয়েছে!" : "Public invoice link copied!");
      } else {
                ...
                <article key={inv.id} className="rounded-2xl border border-border/70 bg-card/60 p-4 space-y-3 shadow-xs">
                  ...
                  <div className="my-3 flex items-end justify-between gap-3 border-y border-border/60 py-3">
                    ...
                    <div className="text-right">
                      <p className="text-[11px] text-muted-foreground">{bn ? "শেষ সময়" : "Due Date"}</p>
